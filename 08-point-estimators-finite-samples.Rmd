...
$$\mathcal{I}(\theta) = E\Big((\frac{\partial}{\partial\theta}\log f(X|\theta))^2\Big|\theta\Big)$$
Under MLE regularity conditions, 

$$\mathcal{I}(\theta) = -E\Big(\frac{\partial^2}{\partial\theta^2}\log f(X|\theta) \Big|\theta\Big)$$
:::

As the mean value of a second derivative, the information measures the curve of $\ell(\theta|X)$ *(expand?)*

There are two ways to estimate the information.

:::{.definition name="Observed Information"}
The observed information is defined as

$$-\frac{\partial^2}{\partial\theta^2}\ell(\theta|X_i)$$
:::
