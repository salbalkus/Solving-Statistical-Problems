...
Let $X$ have $p d f f_X(x)$ and let $Y=g(X)$, where $g$ is a monotone function. Let $\mathcal{X}$ and $\mathcal{Y}$ be defined by (2.1.7). Suppose that $f_X(x)$ is continuous on $\mathcal{X}$ and that $g^{-1}(y)$ has a continuous derivative on $\mathcal{Y}$. Then the PDF of $Y$ is given by
$$
...
$$
<<<<<<< HEAD
For the random variable $Z = X-Y$, the moment generating and characteristic functions are analogously given by:
=======
For the random variable $Z = X-Y$, the moment generating and characteristic functions are given by:
>>>>>>> 111b97e1cc614f657524efe62f33b45cb7fd6858
$$
M_Z(t)=M_X(t) \cdot M_Y(-t)
$$
$$
